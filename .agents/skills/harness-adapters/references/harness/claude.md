...
When the project entry instead carries an explicit decline - `hasClaudeMdExternalIncludesApproved===false` AND `hasClaudeMdExternalIncludesWarningShown===true`, i.e. the dialog was shown and the answer was no - the whole registration refuses, including the trust flag, rather than manufacture consent the human never gave, so that spawn wedges on the trust dialog before it would even reach the import one.
It takes both fields: `approved===false` with `warningShown` false or absent is a question never asked rather than a decline, and lands in the same first-spawn case above, registering trust normally.
