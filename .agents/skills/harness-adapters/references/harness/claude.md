...
When the project entry instead carries an explicit decline - `hasClaudeMdExternalIncludesApproved===false` AND `hasClaudeMdExternalIncludesWarningShown===true`, read as a dialog that was shown and answered no - the whole registration refuses, including the trust flag, rather than manufacture consent the human never gave, so that spawn wedges on the trust dialog before it would even reach the import one.
It takes both fields: `approved===false` with `warningShown` false or absent is treated as a question never asked rather than a decline, and lands in the same first-spawn case above, registering trust normally.
That reading is an inference from observed store entries rather than vendor write behaviour this repo has verified; the script's `declinedExternalImports` predicate comment is the one owner of the pairing, the measurement behind it, and what bounds it being wrong.
