...
The [`fm-spawn.sh` header](../bin/fm-spawn.sh) owns ship/scout worktree isolation and fresh-base refusal rules, including spawns from linked homes, the per-home pool root that keeps two homes' same-named clones out of one Treehouse pool, and the guard that refuses a worktree of any clone but the spawning project's own.
Portable regressions live in [`tests/fm-spawn-pool-base-freshen.test.sh`](../tests/fm-spawn-pool-base-freshen.test.sh) for spawn isolation and base freshness, [`tests/fm-spawn-pool-home.test.sh`](../tests/fm-spawn-pool-home.test.sh) for the per-home pool root and the clone guard, [`tests/fm-treehouse-pool-root.test.sh`](../tests/fm-treehouse-pool-root.test.sh) for the Treehouse pool facts those rely on, and [`tests/fm-control-relaunch.test.sh`](../tests/fm-control-relaunch.test.sh) for preserving the recorded copy on relaunch.
